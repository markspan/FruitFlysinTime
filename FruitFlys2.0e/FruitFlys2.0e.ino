...
void setup() {
  ...
  while (millis() - startTime < 10100)
  {
    ...
    analogWrite(setCopperBlock, i); // slowly ramp up to full cooling
    i = min(i+1, 255);
#ifdef NEXTION
...
void loop() {
  ...
  for (int i = 0; i <= 2; i++)    {
    ...
    if (debug) {
      SerialUSB.print(val2tempTile(analogRead(getTiles[i])));      SerialUSB.print('\t');      SerialUSB.print(tilesPWM[i]);      SerialUSB.print('\t');      SerialUSB.print(tilesDesiredTemp[i]);      SerialUSB.print('\t');
    }
  }
  if (debug)  SerialUSB.print(copperBlockPWM);   SerialUSB.print('\t');  SerialUSB.println(copperBlockMeasuredTemp);
  CopperPID.Compute();
  analogWrite(setCopperBlock, int(copperBlockPWM * 255.0));
  SamplerCB();
}
